                ...
                [
                  fun γ =>
                    ltac:(M.monadic
                      (let γ :=
                        M.use
                          (M.alloc (| BinOp.Pure.gt (M.read (| index |)) (M.read (| len |)) |)) in
                      let _ := M.is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                      M.alloc (|
                        M.never_to_any (|
                          M.call_closure (|
                      ...
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            M.use
                              (M.alloc (|
                                BinOp.Pure.eq (M.read (| original_len |)) (Value.Integer 0)
                              |)) in
                          let _ :=
                            M.is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.alloc (|
            ...
            let~ _ :=
              let β :=
                M.SubPointer.get_struct_record_field (|
                  ...
                  "len"
                |) in
              M.write (| β, BinOp.Wrap.add Integer.Usize (M.read (| β |)) (M.read (| count |)) |) in
            M.alloc (| Value.Tuple [] |)
          |)))
      | _, _, _ => M.impossible
      end.
                    ...
                    M.alloc (|
                      Value.StructRecord
                        ...
                        [
                          ...
                          ("tail_len",
                            BinOp.Wrap.sub Integer.Usize (M.read (| len |)) (M.read (| end_ |)));
                          ("iter",
                            M.call_closure (|
                              M.get_associated_function (|
                                Ty.apply (Ty.path "slice") [] [ T ],
                  ...
                  ltac:(M.monadic
                    ...
                    M.alloc (|
                      M.call_closure (|
                        ...
                        [
                          M.read (| self |);
                          BinOp.Wrap.sub Integer.Usize (M.read (| new_len |)) (M.read (| len |));
                          M.read (| value |)
                        ]
                      |)
                    |)));
