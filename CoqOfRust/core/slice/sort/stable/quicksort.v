                      ...
                      M.loop (|
                        ...
                        ltac:(M.monadic
                          ...
                          let~ pivot_ref :
                            ...
                            |) in
                          let~ perform_equal_partition : Ty.path "bool" := Value.Bool false in
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
            ...
            ltac:(M.monadic
              ...
              let scratch :=
                M.alloc (|
                  ...
                  scratch
                |) in
              let pivot_pos := M.alloc (| Ty.path "usize", pivot_pos |) in
              let pivot_goes_left := M.alloc (| Ty.path "bool", pivot_goes_left |) in
              let is_less := M.alloc (| Ty.apply (Ty.path "&mut") [] [ F ], is_less |) in
              M.read (|
                let~ len : Ty.path "usize" :=
                  M.call_closure (|
                ...
                let~ pivot_in_scratch : Ty.apply (Ty.path "*mut") [] [ T ] :=
                  M.call_closure (|
                    ...
                    []
                  |) in
                let~ loop_end_pos : Ty.path "usize" := M.read (| pivot_pos |) in
                let~ _ : Ty.tuple [] :=
                  M.read (|
                    M.loop (|
                      Ty.tuple [],
                      ltac:(M.monadic
                        ...
                        let~ _ : Ty.tuple [] :=
                          M.write (|
                            ...
                            |)
                          |) in
                        let~ _ : Ty.tuple [] := M.write (| loop_end_pos, M.read (| len |) |) in
                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                    |)
                  |) in
                let~ _ : Ty.tuple [] :=
              ...
              ltac:(M.monadic
                (let self :=
                  M.alloc (|
                    ...
                    self
                  |) in
                let towards_left := M.alloc (| Ty.path "bool", towards_left |) in
                M.read (|
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
