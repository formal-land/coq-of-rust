                      ...
                      M.loop (|
                        ltac:(M.monadic
                          ...
                          let~ _ :=
                            ...
                            |) in
                          let~ left_partition_len := M.alloc (| Value.Integer 0 |) in
                          let~ _ :=
                            M.match_operator (|
                              M.alloc (| Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.use
                                        (M.alloc (|
                                          UnOp.Pure.not (M.read (| perform_equal_partition |))
                                        |)) in
                                    let _ :=
                                      M.is_constant_or_break_match (|
                                        M.read (| γ |),
                                  ...
                                  [
                                    LogicalOp.or (|
                                      BinOp.Pure.lt
                                        ...
                                        (M.read (| len |)),
                                      ltac:(M.monadic
                                        (BinOp.Pure.ge (M.read (| pivot_pos |)) (M.read (| len |))))
                                    |)
                                  ]
                                |)
                              |)) in
                            ...
                            fun γ =>
                              ltac:(M.monadic
                                (let γ :=
                                  M.use
                                    (M.alloc (|
                                      BinOp.Pure.eq (M.read (| loop_end_pos |)) (M.read (| len |))
                                    |)) in
                                let _ :=
                                  M.is_constant_or_break_match (|
                                    M.read (| γ |),
                          ...
                          let~ _ :=
                            M.alloc (|
                              M.call_closure (|
                                ...
                                [ M.read (| pivot |); M.read (| pivot_in_scratch |); Value.Integer 1
                                ]
                              |)
                            |) in
                          M.alloc (| Value.Tuple [] |)));
                  ...
                  let~ _ :=
                    ...
                    M.write (|
                      ...
                      BinOp.Wrap.add
                        Integer.Usize
                        (M.read (| β |))
                        (M.rust_cast (M.read (| towards_left |)))
                    |) in
                  let~ _ :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
