...
Definition bool_and (a b : Bool.t) : Bool.t :=
  if a.(Bool.value)
  then if b.(Bool.value) then Bool.true else Bool.false
  else Bool.false.

Lemma bool_and_eq (a b : Bool.t) :
  {{
...
Proof.
  ...
  repeat (
    ...
    eapply Run.Call ||
    apply Run.Pure ||
    destruct (a.(Bool.value)) ||
    destruct (b.(Bool.value))
  ).
Qed.
...
Definition eq2 (a b : array.t U32.t {| Integer.value := 2 |}) : Bool.t :=
...
Definition eq_pair (x y : U32.t * U32.t) : Bool.t :=
  let '(x0, x1) := x in
  let '(y0, y1) := y in
  if (x0.(Integer.value) =? y0.(Integer.value)) &&
     (x1.(Integer.value) =? y1.(Integer.value))
