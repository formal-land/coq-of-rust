...
Lemma apply_duplicate_eq_bis (numbers : Numbers.t) :
  let ref_numbers :=
    {| Ref.core := Ref.Core.Mutable (A := Numbers.t) 0%nat [] φ Some (fun _ => Some) |} in
  {{
    StackM.eval_f (Stack := [_]) (run_apply_duplicate ref_numbers) numbers 🌲
...
Proof.
  cbn; repeat get_can_access; eapply Run.Call. {
    cbn; repeat get_can_access; apply Run.Pure.
  }
  cbn; repeat get_can_access; eapply Run.Call. {
    cbn; repeat get_can_access; apply Run.Pure.
  }
  cbn; repeat get_can_access; eapply Run.Call; cbn. {
    Show.
