  ...
  ltac:(M.monadic (
    M.read (|
      let twenty :
        ...
        M.alloc (|
          M.call (|(map_in_result_via_combinators.multiply
            (M.read (| mk_str "10" |))
            (M.read (| mk_str "2" |)))
          |)
        |) in
      let _ : M.Val unit :=
        M.alloc (|
          M.call (|(map_in_result_via_combinators.print (M.read (| twenty |)))
          |)
        |) in
      let tt_ :
          M.Val (core.result.Result.t i32.t core.num.error.ParseIntError.t) :=
      ...
      let _ : M.Val unit :=
        M.alloc (|
          M.call (|(map_in_result_via_combinators.print (M.read (| tt_ |))) |)
        |) in
      M.alloc (| tt |)
    |)
  )).
