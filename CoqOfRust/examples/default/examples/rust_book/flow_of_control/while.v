...
Definition main (𝜏 : list Ty.t) (α : list Value.t) : M :=
  match 𝜏, α with
  | [], [] =>
    let* n : Ty.path "i32" := M.alloc ((Integer.of_Z 1) : Ty.path "i32") in
    let* α0 : Ty.tuple :=
      M.loop
        (let* α0 : Ty.path "i32" := M.read n in
        let* α1 : Ty.path "bool" :=
          M.alloc (BinOp.Pure.lt α0 ((Integer.of_Z 101) : Ty.path "i32")) in
        let* α2 : Ty.path "bool" := M.read (use α1) in
        if α2 then
          let* _ : Ty.tuple :=
            let* α0 : Ty.path "i32" := M.read n in
            let* α1 : Ty.path "i32" :=
              BinOp.Panic.rem α0 ((Integer.of_Z 15) : Ty.path "i32") in
            let* α2 : Ty.path "bool" :=
...
") in
              ...
              let* α0 : Ty.path "i32" := M.read n in
              let* α1 : Ty.path "i32" :=
                BinOp.Panic.rem α0 ((Integer.of_Z 3) : Ty.path "i32") in
              let* α2 : Ty.path "bool" :=
                M.alloc (BinOp.Pure.eq α1 ((Integer.of_Z 0) : Ty.path "i32")) in
              let* α3 : Ty.path "bool" := M.read (use α2) in
              if α3 then
...
") in
                ...
                let* α0 : Ty.path "i32" := M.read n in
                let* α1 : Ty.path "i32" :=
                  BinOp.Panic.rem α0 ((Integer.of_Z 5) : Ty.path "i32") in
                let* α2 : Ty.path "bool" :=
                  M.alloc
                    (BinOp.Pure.eq α1 ((Integer.of_Z 0) : Ty.path "i32")) in
                let* α3 : Ty.path "bool" := M.read (use α2) in
                if α3 then
                  let* _ : Ty.tuple :=
                    let* _ : Ty.tuple :=
