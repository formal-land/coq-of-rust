(* Generated by coq-of-rust *)
Require Import CoqOfRust.CoqOfRust.

(*
fn main() {
    let vec1 = vec![1, 2, 3];
    let vec2 = vec![4, 5, 6];

    // `iter()` for vecs yields `&i32`. Destructure to `i32`.
    println!("2 in vec1: {}", vec1.iter().any(|&x| x == 2));
    // `into_iter()` for vecs yields `i32`. No destructuring required.
    println!("2 in vec2: {}", vec2.into_iter().any(|x| x == 2));

    // `iter()` only borrows `vec1` and its elements, so they can be used again
    println!("vec1 len: {}", vec1.len());
    println!("First element of vec1 is: {}", vec1[0]);
    // `into_iter()` does move `vec2` and its elements, so they cannot be used again
    // println!("First element of vec2 is: {}", vec2[0]);
    // println!("vec2 len: {}", vec2.len());
    // TODO: uncomment two lines above and see compiler errors.

    let array1 = [1, 2, 3];
    let array2 = [4, 5, 6];

    // `iter()` for arrays yields `&i32`.
    println!("2 in array1: {}", array1.iter().any(|&x| x == 2));
    // `into_iter()` for arrays yields `i32`.
    println!("2 in array2: {}", array2.into_iter().any(|x| *x == 2));
}
*)
(* #[allow(dead_code)] - function was ignored by the compiler *)
Definition main (𝜏 : list Ty.t) (α : list Value.t) : M :=
  match 𝜏, α with
  | [], [] =>
    let* vec1 :
        Ty.apply
          (Ty.path "alloc::vec::Vec")
          [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
      let* α0 : Ty.apply (Ty.path "array") [Ty.path "i32"] :=
        M.alloc
          [
            (Integer.of_Z 1) : Ty.path "i32";
            (Integer.of_Z 2) : Ty.path "i32";
            (Integer.of_Z 3) : Ty.path "i32"
          ] in
      let* α1 :
          Ty.apply
            (Ty.path "alloc::boxed::Box")
            [Ty.apply (Ty.path "array") [Ty.path "i32"];
              Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.call ((alloc.boxed.Box.t _ alloc.boxed.Box.Default.A)::["new"] α0) in
      let* α2 :
          Ty.apply
            (Ty.path "alloc::boxed::Box")
            [Ty.apply (Ty.path "array") [Ty.path "i32"];
              Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.read α1 in
      let* α3 :
          Ty.apply
            (Ty.path "alloc::vec::Vec")
            [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.call
          ((Ty.apply (Ty.path "slice") [Ty.path "i32"])::["into_vec"]
            (pointer_coercion "Unsize" α2)) in
      M.alloc α3 in
    let* vec2 :
        Ty.apply
          (Ty.path "alloc::vec::Vec")
          [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
      let* α0 : Ty.apply (Ty.path "array") [Ty.path "i32"] :=
        M.alloc
          [
            (Integer.of_Z 4) : Ty.path "i32";
            (Integer.of_Z 5) : Ty.path "i32";
            (Integer.of_Z 6) : Ty.path "i32"
          ] in
      let* α1 :
          Ty.apply
            (Ty.path "alloc::boxed::Box")
            [Ty.apply (Ty.path "array") [Ty.path "i32"];
              Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.call ((alloc.boxed.Box.t _ alloc.boxed.Box.Default.A)::["new"] α0) in
      let* α2 :
          Ty.apply
            (Ty.path "alloc::boxed::Box")
            [Ty.apply (Ty.path "array") [Ty.path "i32"];
              Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.read α1 in
      let* α3 :
          Ty.apply
            (Ty.path "alloc::vec::Vec")
            [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
        M.call
          ((Ty.apply (Ty.path "slice") [Ty.path "i32"])::["into_vec"]
            (pointer_coercion "Unsize" α2)) in
      M.alloc α3 in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "2 in vec1: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 :
            Ty.function
              [Ty.apply
                  (Ty.path "mut_ref")
                  [Ty.apply
                      (Ty.path "core::slice::iter::Iter")
                      [Ty.path "i32"]];
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool")]
              (Ty.path "bool") :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.iterator.Iterator.any
              (Self :=
                Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"])
              (F :=
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool"))
              (Trait := ℐ))) in
        let* α4 :
            Ty.function
              [Ty.apply
                  (Ty.path "ref")
                  [Ty.apply
                      (Ty.path "alloc::vec::Vec")
                      [Ty.path "i32";
                        Ty.apply (Ty.path "alloc::alloc::Global") []]]]
              (Ty.apply (Ty.path "ref") [_]) :=
          ltac:(M.get_method (fun ℐ =>
            core.ops.deref.Deref.deref
              (Self :=
                Ty.apply
                  (Ty.path "alloc::vec::Vec")
                  [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []])
              (Trait := ℐ))) in
        let* α5 :
            Ty.apply
              (Ty.path "ref")
              [Ty.apply (Ty.path "slice") [Ty.path "i32"]] :=
          M.call (α4 (borrow vec1)) in
        let* α6 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.call ((Ty.apply (Ty.path "slice") [Ty.path "i32"])::["iter"] α5) in
        let* α7 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.alloc α6 in
        let* α8 : Ty.path "bool" :=
          M.call
            (α3
              (borrow_mut α7)
              (fun (α0 : Ty.apply (Ty.path "ref") [Ty.path "i32"]) =>
                (let* α0 := M.alloc α0 in
                match_operator
                  α0
                  [
                    fun γ =>
                      (let* γ :=
                        let* α0 := M.read γ in
                        M.pure (deref α0) in
                      let* x := M.copy γ in
                      let* α0 : Ty.path "i32" := M.read x in
                      M.pure
                        (BinOp.Pure.eq α0 ((Integer.of_Z 2) : Ty.path "i32"))) :
                      Ty.path "bool"
                  ]) :
                Ty.path "bool")) in
        let* α9 : Ty.path "bool" := M.alloc α8 in
        let* α10 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              (borrow α9)) in
        let* α11 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α10 ] in
        let* α12 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α11))) in
        let* α13 : Ty.tuple := M.call (std.io.stdio._print α12) in
        M.alloc α13 in
      M.alloc tt in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "2 in vec2: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 :
            Ty.function
              [Ty.apply
                  (Ty.path "mut_ref")
                  [Ty.apply
                      (Ty.path "alloc::vec::into_iter::IntoIter")
                      [Ty.path "i32";
                        Ty.apply (Ty.path "alloc::alloc::Global") []]];
                Ty.function [Ty.tuple (Ty.path "i32")] (Ty.path "bool")]
              (Ty.path "bool") :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.iterator.Iterator.any
              (Self :=
                Ty.apply
                  (Ty.path "alloc::vec::into_iter::IntoIter")
                  [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []])
              (F := Ty.function [Ty.tuple (Ty.path "i32")] (Ty.path "bool"))
              (Trait := ℐ))) in
        let* α4 :
            Ty.function
              [Ty.apply
                  (Ty.path "alloc::vec::Vec")
                  [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []]]
              _ :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.collect.IntoIterator.into_iter
              (Self :=
                Ty.apply
                  (Ty.path "alloc::vec::Vec")
                  [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []])
              (Trait := ℐ))) in
        let* α5 :
            Ty.apply
              (Ty.path "alloc::vec::Vec")
              [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
          M.read vec2 in
        let* α6 :
            Ty.apply
              (Ty.path "alloc::vec::into_iter::IntoIter")
              [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
          M.call (α4 α5) in
        let* α7 :
            Ty.apply
              (Ty.path "alloc::vec::into_iter::IntoIter")
              [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []] :=
          M.alloc α6 in
        let* α8 : Ty.path "bool" :=
          M.call
            (α3
              (borrow_mut α7)
              (fun (α0 : Ty.path "i32") =>
                (let* α0 := M.alloc α0 in
                match_operator
                  α0
                  [
                    fun γ =>
                      (let* x := M.copy γ in
                      let* α0 : Ty.path "i32" := M.read x in
                      M.pure
                        (BinOp.Pure.eq α0 ((Integer.of_Z 2) : Ty.path "i32"))) :
                      Ty.path "bool"
                  ]) :
                Ty.path "bool")) in
        let* α9 : Ty.path "bool" := M.alloc α8 in
        let* α10 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              (borrow α9)) in
        let* α11 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α10 ] in
        let* α12 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α11))) in
        let* α13 : Ty.tuple := M.call (std.io.stdio._print α12) in
        M.alloc α13 in
      M.alloc tt in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "vec1 len: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 : Ty.path "usize" :=
          M.call
            ((Ty.apply
                  (Ty.path "alloc::vec::Vec")
                  [Ty.path "i32";
                    Ty.apply (Ty.path "alloc::alloc::Global") []])::["len"]
              (borrow vec1)) in
        let* α4 : Ty.path "usize" := M.alloc α3 in
        let* α5 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              (borrow α4)) in
        let* α6 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α5 ] in
        let* α7 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α6))) in
        let* α8 : Ty.tuple := M.call (std.io.stdio._print α7) in
        M.alloc α8 in
      M.alloc tt in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "First element of vec1 is: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 :
            Ty.function
              [Ty.apply
                  (Ty.path "ref")
                  [Ty.apply
                      (Ty.path "alloc::vec::Vec")
                      [Ty.path "i32";
                        Ty.apply (Ty.path "alloc::alloc::Global") []]];
                Ty.path "usize"]
              (Ty.apply (Ty.path "ref") [_]) :=
          ltac:(M.get_method (fun ℐ =>
            core.ops.index.Index.index
              (Self :=
                Ty.apply
                  (Ty.path "alloc::vec::Vec")
                  [Ty.path "i32"; Ty.apply (Ty.path "alloc::alloc::Global") []])
              (Idx := Ty.path "usize")
              (Trait := ℐ))) in
        let* α4 : Ty.apply (Ty.path "ref") [Ty.path "i32"] :=
          M.call (α3 (borrow vec1) ((Integer.of_Z 0) : Ty.path "usize")) in
        let* α5 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              α4) in
        let* α6 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α5 ] in
        let* α7 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α6))) in
        let* α8 : Ty.tuple := M.call (std.io.stdio._print α7) in
        M.alloc α8 in
      M.alloc tt in
    let* array1 : Ty.apply (Ty.path "array") [Ty.path "i32"] :=
      M.alloc
        [
          (Integer.of_Z 1) : Ty.path "i32";
          (Integer.of_Z 2) : Ty.path "i32";
          (Integer.of_Z 3) : Ty.path "i32"
        ] in
    let* array2 : Ty.apply (Ty.path "array") [Ty.path "i32"] :=
      M.alloc
        [
          (Integer.of_Z 4) : Ty.path "i32";
          (Integer.of_Z 5) : Ty.path "i32";
          (Integer.of_Z 6) : Ty.path "i32"
        ] in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "2 in array1: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 :
            Ty.function
              [Ty.apply
                  (Ty.path "mut_ref")
                  [Ty.apply
                      (Ty.path "core::slice::iter::Iter")
                      [Ty.path "i32"]];
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool")]
              (Ty.path "bool") :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.iterator.Iterator.any
              (Self :=
                Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"])
              (F :=
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool"))
              (Trait := ℐ))) in
        let* α4 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.call
            ((Ty.apply (Ty.path "slice") [Ty.path "i32"])::["iter"]
              (pointer_coercion "Unsize" (borrow array1))) in
        let* α5 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.alloc α4 in
        let* α6 : Ty.path "bool" :=
          M.call
            (α3
              (borrow_mut α5)
              (fun (α0 : Ty.apply (Ty.path "ref") [Ty.path "i32"]) =>
                (let* α0 := M.alloc α0 in
                match_operator
                  α0
                  [
                    fun γ =>
                      (let* γ :=
                        let* α0 := M.read γ in
                        M.pure (deref α0) in
                      let* x := M.copy γ in
                      let* α0 : Ty.path "i32" := M.read x in
                      M.pure
                        (BinOp.Pure.eq α0 ((Integer.of_Z 2) : Ty.path "i32"))) :
                      Ty.path "bool"
                  ]) :
                Ty.path "bool")) in
        let* α7 : Ty.path "bool" := M.alloc α6 in
        let* α8 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              (borrow α7)) in
        let* α9 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α8 ] in
        let* α10 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α9))) in
        let* α11 : Ty.tuple := M.call (std.io.stdio._print α10) in
        M.alloc α11 in
      M.alloc tt in
    let* _ : Ty.tuple :=
      let* _ : Ty.tuple :=
        let* α0 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "2 in array2: ") in
        let* α1 : Ty.apply (Ty.path "ref") [Ty.path "str"] :=
          M.read (mk_str "
") in
        let* α2 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "ref") [Ty.path "str"]] :=
          M.alloc [ α0; α1 ] in
        let* α3 :
            Ty.function
              [Ty.apply
                  (Ty.path "mut_ref")
                  [Ty.apply
                      (Ty.path "core::slice::iter::Iter")
                      [Ty.path "i32"]];
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool")]
              (Ty.path "bool") :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.iterator.Iterator.any
              (Self :=
                Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"])
              (F :=
                Ty.function
                  [Ty.tuple (Ty.apply (Ty.path "ref") [Ty.path "i32"])]
                  (Ty.path "bool"))
              (Trait := ℐ))) in
        let* α4 :
            Ty.function
              [Ty.apply
                  (Ty.path "ref")
                  [Ty.apply (Ty.path "array") [Ty.path "i32"]]]
              _ :=
          ltac:(M.get_method (fun ℐ =>
            core.iter.traits.collect.IntoIterator.into_iter
              (Self :=
                Ty.apply
                  (Ty.path "ref")
                  [Ty.apply (Ty.path "array") [Ty.path "i32"]])
              (Trait := ℐ))) in
        let* α5 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.call (α4 (borrow array2)) in
        let* α6 :
            Ty.apply (Ty.path "core::slice::iter::Iter") [Ty.path "i32"] :=
          M.alloc α5 in
        let* α7 : Ty.path "bool" :=
          M.call
            (α3
              (borrow_mut α6)
              (fun (α0 : Ty.apply (Ty.path "ref") [Ty.path "i32"]) =>
                (let* α0 := M.alloc α0 in
                match_operator
                  α0
                  [
                    fun γ =>
                      (let* x := M.copy γ in
                      let* α0 : Ty.apply (Ty.path "ref") [Ty.path "i32"] :=
                        M.read x in
                      let* α1 : Ty.path "i32" := M.read (deref α0) in
                      M.pure
                        (BinOp.Pure.eq α1 ((Integer.of_Z 2) : Ty.path "i32"))) :
                      Ty.path "bool"
                  ]) :
                Ty.path "bool")) in
        let* α8 : Ty.path "bool" := M.alloc α7 in
        let* α9 : Ty.apply (Ty.path "core::fmt::rt::Argument") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::rt::Argument") [])::["new_display"]
              (borrow α8)) in
        let* α10 :
            Ty.apply
              (Ty.path "array")
              [Ty.apply (Ty.path "core::fmt::rt::Argument") []] :=
          M.alloc [ α9 ] in
        let* α11 : Ty.apply (Ty.path "core::fmt::Arguments") [] :=
          M.call
            ((Ty.apply (Ty.path "core::fmt::Arguments") [])::["new_v1"]
              (pointer_coercion "Unsize" (borrow α2))
              (pointer_coercion "Unsize" (borrow α10))) in
        let* α12 : Ty.tuple := M.call (std.io.stdio._print α11) in
        M.alloc α12 in
      M.alloc tt in
    let* α0 : Ty.path "unit" := M.alloc tt in
    M.read α0
  | _, _ => M.impossible
  end.
