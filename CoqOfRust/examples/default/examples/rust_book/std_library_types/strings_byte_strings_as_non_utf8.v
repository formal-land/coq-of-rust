...
Definition main (τ : list Ty.t) (α : list Value.t) : M :=
  match τ, α with
  | [], [] =>
    let* bytestring := let* α0 := M.read UnsupportedLiteral in M.alloc α0 in
    let* _ :=
      let* _ :=
        ...
        [
          fun γ =>
            let* γ :=
              ...
              let* α2 :=
                (* Unsize *) let* α1 := M.read raw_bytestring in M.pure (M.pointer_coercion α1) in
...
") in
    ...
    let* _ :=
      let* α0 := M.get_function "core::str::converts::from_utf8" [] in
      let* α2 := (* Unsize *) let* α1 := M.read shift_jis in M.pure (M.pointer_coercion α1) in
