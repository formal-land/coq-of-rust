  ...
  *)
  Definition fmt
    ...
    M.function_body
      ...
      let* α1 : M.Val core.fmt.Formatter.t := deref α0 in
      let* α2 : mut_ref core.fmt.Formatter.t := borrow_mut α1 in
      let* α3 : ref str.t := M.read (mk_str "Pair") in
      let* α4 : M.Val str.t := deref α3 in
      let* α5 : ref str.t := borrow α4 in
      let* α6 : ref clone.Pair.t := M.read self in
      let* α7 : M.Val clone.Pair.t := deref α6 in
      let* α8 : ref (alloc.boxed.Box.t i32.t alloc.alloc.Global.t) :=
        borrow α7.["0"] in
      let* α9 : M.Val (ref (alloc.boxed.Box.t i32.t alloc.alloc.Global.t)) :=
        M.alloc α8 in
      let* α10 : M.Val (ref type not implemented) :=
        pointer_coercion "Unsize" α9 in
      let* α11 : ref type not implemented := M.read α10 in
      let* α12 : ref clone.Pair.t := M.read self in
      let* α13 : M.Val clone.Pair.t := deref α12 in
      let* α14 : ref (alloc.boxed.Box.t i32.t alloc.alloc.Global.t) :=
        borrow α13.["1"] in
      let* α15 : M.Val (ref (alloc.boxed.Box.t i32.t alloc.alloc.Global.t)) :=
        M.alloc α14 in
      let* α16 : ref (ref (alloc.boxed.Box.t i32.t alloc.alloc.Global.t)) :=
        borrow α15 in
      let* α17 :
      ...
      core.fmt.Formatter.t::["debug_tuple_field2_finish"] α2 α5 α11 α19).
