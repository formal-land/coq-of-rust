...
Definition main : M unit :=
  ...
  let* vec : M.Val (alloc.vec.Vec.t u8.t alloc.alloc.Global.t) :=
    let* α0 : alloc.vec.Vec.t u8.t alloc.alloc.Global.t :=
      M.call (impl (alloc.vec.Vec.t u8.t alloc.alloc.Global.t) "new") in
    M.alloc α0 in
  let* _ : M.Val unit :=
    let* α0 : u8.t := M.read elem in
    let* α1 : unit :=
      M.call
        (impl (alloc.vec.Vec.t u8.t alloc.alloc.Global.t) "push"
          (borrow_mut vec)
          α0) in
    M.alloc α1 in
  let* _ : M.Val unit :=
...
") in
      ...
      let* α5 : core.fmt.rt.Argument.t :=
        M.call (impl core.fmt.rt.Argument.t "new_debug" (borrow vec)) in
