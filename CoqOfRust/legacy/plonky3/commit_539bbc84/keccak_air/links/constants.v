...
Definition R : array.t (array.t U8.t {| Integer.value := 5 |}) {| Integer.value := 5 |} :=
  array.Build_t _ {| Integer.value := 5 |} (ArrayPairs.of_list [
    ...
    array.Build_t _ {| Integer.value := 5 |} (
      ArrayPairs.of_list (List.map (Integer.Build_t IntegerKind.U8) [1; 44; 10; 45; 2])
    );
    array.Build_t _ {| Integer.value := 5 |} (
      ArrayPairs.of_list (List.map (Integer.Build_t IntegerKind.U8) [62; 6; 43; 15; 61])
    );
    array.Build_t _ {| Integer.value := 5 |} (
      ArrayPairs.of_list (List.map (Integer.Build_t IntegerKind.U8) [28; 55; 25; 21; 56])
    );
    array.Build_t _ {| Integer.value := 5 |} (
      ArrayPairs.of_list (List.map (Integer.Build_t IntegerKind.U8) [27; 20; 39; 8; 14])
    )
  ]).
...
*)
Definition RC_list : list (Integer.t IntegerKind.U64) :=
  List.map (Integer.Build_t IntegerKind.U64) [
    0x0000000000000001;
    0x0000000000008082;
    0x800000000000808A;
    0x8000000080008000;
...
Definition RC_BITS : array.t (array.t U8.t {| Integer.value := 64 |}) {| Integer.value := 24 |} :=
  array.Build_t _ {| Integer.value := 24 |} (
    ArrayPairs.of_list (
      List.map
        (fun x => array.Build_t _ {| Integer.value := 64 |} (
          ArrayPairs.of_list (
            List.map
