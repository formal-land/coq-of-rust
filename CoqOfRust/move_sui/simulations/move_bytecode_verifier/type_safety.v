...
Definition verify_instr (verifier : TypeSafetyChecker.t) (bytecode : Bytecode.t) 
  ...
  | Bytecode.BrTrue idx => Result.Ok tt
  | Bytecode.BrFalse idx => Result.Ok tt
  | Bytecode.Branch idx => Result.Ok tt
  | Bytecode.LdU8 idx => Result.Ok tt
  | Bytecode.LdU64 idx => Result.Ok tt
  | Bytecode.LdU128 idx => Result.Ok tt
  | Bytecode.CastU8 => Result.Ok tt
  | Bytecode.CastU64 => Result.Ok tt
  | Bytecode.CastU128 => Result.Ok tt
  | Bytecode.LdConst idx => Result.Ok tt
  | Bytecode.LdTrue => Result.Ok tt
  | Bytecode.LdFalse => Result.Ok tt
  | Bytecode.CopyLoc idx => Result.Ok tt
  | Bytecode.MoveLoc idx => Result.Ok tt
  | Bytecode.StLoc idx => Result.Ok tt
  | Bytecode.Call idx => Result.Ok tt
  | Bytecode.CallGeneric idx => Result.Ok tt
  | Bytecode.Pack idx => Result.Ok tt
  | Bytecode.PackGeneric idx => Result.Ok tt
  | Bytecode.Unpack idx => Result.Ok tt
  | Bytecode.UnpackGeneric idx => Result.Ok tt
  | Bytecode.ReadRef => Result.Ok tt
  | Bytecode.WriteRef => Result.Ok tt
  | Bytecode.FreezeRef => Result.Ok tt
  | Bytecode.MutBorrowLoc idx => Result.Ok tt
  | Bytecode.ImmBorrowLoc idx => Result.Ok tt
  | Bytecode.MutBorrowField idx => Result.Ok tt
  | Bytecode.MutBorrowFieldGeneric idx => Result.Ok tt
  | Bytecode.ImmBorrowField idx => Result.Ok tt
  | Bytecode.ImmBorrowFieldGeneric idx => Result.Ok tt
  | Bytecode.MutBorrowGlobal idx => Result.Ok tt
  | Bytecode.MutBorrowGlobalGeneric idx => Result.Ok tt
  | Bytecode.ImmBorrowGlobal idx => Result.Ok tt
  | Bytecode.ImmBorrowGlobalGeneric idx => Result.Ok tt
  ...
  | Bytecode.Exists idx => Result.Ok tt
  | Bytecode.ExistsGeneric idx => Result.Ok tt
  | Bytecode.MoveFrom idx => Result.Ok tt
  | Bytecode.MoveFromGeneric idx => Result.Ok tt
  | Bytecode.MoveTo idx => Result.Ok tt
  | Bytecode.MoveToGeneric idx => Result.Ok tt
  | Bytecode.Shl => Result.Ok tt
  | Bytecode.Shr => Result.Ok tt
  (* | Bytecode.VecPack (_ : SignatureIndex.t) (_ : Z) => Result.Ok tt *)
  | Bytecode.VecLen idx => Result.Ok tt
  | Bytecode.VecImmBorrow idx => Result.Ok tt
  | Bytecode.VecMutBorrow idx => Result.Ok tt
  | Bytecode.VecPushBack idx => Result.Ok tt
  | Bytecode.VecPopBack idx => Result.Ok tt
  (* | Bytecode.VecUnpack (_ : SignatureIndex.t) (_ : Z) => Result.Ok tt *)
  | Bytecode.VecSwap idx => Result.Ok tt
  | Bytecode.LdU16 idx => Result.Ok tt
  | Bytecode.LdU32 idx => Result.Ok tt
  | Bytecode.LdU256 idx => Result.Ok tt
