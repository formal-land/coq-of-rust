...
Module Impl_ruint_Uint_BITS_LIMBS.
  ...
  *)
      ...
      (let~ limbs : Ty.path "usize" :=
        M.alloc (|
          M.call_closure (|
            ...
            [ M.read (| get_constant (| "ruint::BITS", Ty.path "usize" |) |) ]
          |)
        |) in
      let~ _ : Ty.tuple [] :=
        M.match_operator (|
                ...
                (let γ :=
                  M.use
                    (M.alloc (|
                      UnOp.not (|
                        BinOp.eq (|
                          M.read (| get_constant (| "ruint::LIMBS", Ty.path "usize" |) |),
                          M.read (| limbs |)
                        |)
                      |)
                    |)) in
  ...
  Definition value_BITS
      ...
      : M :=
    let Self : Ty.t := Self BITS LIMBS in
    ltac:(M.monadic (get_constant (| "ruint::BITS", Ty.path "usize" |))).
            ...
            [
              fun γ =>
                ltac:(M.monadic
                  ...
                  let _ := M.is_constant_or_break_match (| M.read (| γ0_1 |), Value.Bool false |) in
                  n));
              fun γ =>
                ltac:(M.monadic
                  (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                                    ...
                                    (let γ :=
                                      M.use
                                        (M.alloc (|
                                          BinOp.gt (|
                                            M.read (|
                                              get_constant (| "ruint::LIMBS", Ty.path "usize" |)
                                            |),
                                            Value.Integer IntegerKind.Usize 0
                                          |)
                                        |)) in
...
Definition nlimbs (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
  ...
  | [], [], [ bits ] =>
    ltac:(M.monadic
      ...
      BinOp.Wrap.div (|
        BinOp.Wrap.add (| M.read (| bits |), Value.Integer IntegerKind.Usize 63 |),
              ...
              [
                ...
                fun γ =>
                  ltac:(M.monadic
                    (M.alloc (|
                      BinOp.Wrap.sub (|
                        BinOp.Wrap.shl (| Value.Integer IntegerKind.U64 1, M.read (| bits |) |),
