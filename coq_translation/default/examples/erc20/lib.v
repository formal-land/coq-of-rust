...
Section PureMap.
  ...
  Axiom get_empty : forall k, get k empty = None.
  Axiom get_insert_eq : forall k v m, get k (insert k v m) = Some v.
...
(** We define the [Mapping] type with a tactic to be able to directly access to
    the value types behind [K] and [V]. *)
Ltac Mapping K V :=
  with_strategy opaque [M.Val]
    match eval hnf in K with
    | M.Val ?K =>
      match eval cbv in V with
      | M.Val ?V => exact (M.Val (PureMap.t K V))
      end
    end.
...
Section Impl_core_default_Default_for_lib_Mapping_K_V.
  ...
  Definition default : M (ltac:(lib.Mapping (M.Val K) (M.Val V))) :=
    M.alloc PureMap.empty.
...
Section Impl_lib_Mapping_K_V.
  ...
  Definition get (self : ref Self) (key : ref (M.Val K)) :
      M (core.option.Option (M.Val V)) :=
    let* self : Self := deref self in
    let* self : PureMap.t K V := M.read self in
    let* key : M.Val K := deref key in
    let* key : K := M.read key in
    match PureMap.get key self with
    | None => M.alloc core.option.Option.None
    | Some value =>
      let* value := M.alloc value in
      M.alloc (core.option.Option.Some value)
    end.
  ...
  Definition insert (self : mut_ref Self) (key : M.Val K) (value : M.Val V) :
      M unit :=
    let* self : Self := deref self in
    let* self_value : PureMap.t K V := M.read self in
    let* key := M.read key in
    let* value := M.read value in
    let* _ := M.write self (PureMap.insert key value self_value) in
    M.alloc tt.
    ...
    M.function_body
      (let* α0 : ltac:(refine u128) :=
        core.default.Default.default
          (Self := u128)
          (Trait := ltac:(refine _)) in
      let* α1 : ltac:(refine (ltac:(lib.Mapping lib.AccountId u128))) :=
        core.default.Default.default
          (Self := ltac:(lib.Mapping lib.AccountId u128))
          (Trait := ltac:(refine _)) in
      let* α2 :
          ltac:(refine
            (ltac:(lib.Mapping (M.Val (lib.AccountId * lib.AccountId)) u128))) :=
        core.default.Default.default
          (Self := ltac:(lib.Mapping (M.Val (lib.AccountId * lib.AccountId)) u128))
          (Trait := ltac:(refine _)) in
      M.alloc
        {|
...
Module  Impl_lib_Erc20_2.
  ...
  Definition new (total_supply : ltac:(lib.Balance)) : M Self :=
    M.function_body
      (let* balances : ltac:(refine (ltac:(lib.Mapping lib.AccountId u128))) :=
        core.default.Default.default
          (Self := ltac:(lib.Mapping lib.AccountId u128))
          (Trait := ltac:(refine _)) in
      ...
      let* _ : ltac:(refine unit) :=
        let* α0 : ltac:(refine (mut_ref (ltac:(lib.Mapping lib.AccountId u128)))) :=
          borrow_mut balances in
        (ltac:(lib.Mapping lib.AccountId u128))::["insert"] α0 caller total_supply in
      let* _ : ltac:(refine unit) :=
        let* α0 : ltac:(refine lib.Environment) := lib.Erc20::["init_env"] in
        let* α1 : ltac:(refine (ref lib.Environment)) := borrow α0 in
  ...
  Definition balance_of_impl
    ...
    M.function_body
      ...
      let* α1 : ltac:(refine (ltac:(lib.Mapping lib.AccountId u128))) :=
        α0.["balances"] in
      let* α2 : ltac:(refine (ref (ltac:(lib.Mapping lib.AccountId u128)))) :=
        borrow α1 in
      ...
      let* α5 : ltac:(refine (core.option.Option u128)) :=
        (ltac:(lib.Mapping lib.AccountId u128))::["get"] α2 α4 in
  ...
  Definition allowance_impl
    ...
    M.function_body
      ...
      let* α2 :
          ltac:(refine
            (ref (ltac:(lib.Mapping (M.Val (lib.AccountId * lib.AccountId)) u128)))) :=
        borrow α1 in
      let* α3 : ltac:(refine lib.AccountId) := deref owner in
      ...
      let* α7 : ltac:(refine (core.option.Option u128)) :=
        (ltac:(lib.Mapping (M.Val (lib.AccountId * lib.AccountId)) u128))::["get"]
          α2
          α6 in
  ...
  Definition transfer_from_to
    ...
    M.function_body
      ...
      let* _ : ltac:(refine unit) :=
        ...
        let* α1 : ltac:(refine (ltac:(lib.Mapping lib.AccountId u128))) :=
          α0.["balances"] in
        let* α2 : ltac:(refine (mut_ref (ltac:(lib.Mapping lib.AccountId u128)))) :=
          borrow_mut α1 in
        let* α3 : ltac:(refine lib.AccountId) := deref from in
        let* α4 : ltac:(refine u128) := BinOp.sub from_balance value in
        (ltac:(lib.Mapping lib.AccountId u128))::["insert"] α2 α3 α4 in
  ...
  Definition approve
    ...
    M.function_body
      ...
      let* _ : ltac:(refine unit) :=
        ...
        (ltac:(lib.Mapping (M.Val (lib.AccountId * lib.AccountId)) u128))::["insert"]
          α2
          α3
          value in
      let* _ : ltac:(refine unit) :=
