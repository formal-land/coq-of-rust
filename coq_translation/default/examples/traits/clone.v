...
Module Impl_clone_Speak_for_Tuple_A_bool_.
  Section Impl_clone_Speak_for_Tuple_A_bool_.
    ...
    Definition speak `{State.Trait} (self : ref Self) : M unit :=
      let* _ :=
        let* α0 := borrow "Unknown Field" _ in
        clone.Speak.speak α0 in
      let* _ :=
        let* _ :=
          let* α0 := borrow [ mk_str "bool: "; mk_str "
" ] (list (ref str)) in
          let* α1 := deref α0 (list (ref str)) in
          let* α2 := borrow α1 (list (ref str)) in
          let* α3 := pointer_coercion "Unsize" α2 in
          let* α4 := borrow "Unknown Field" bool in
...
Module Impl_clone_Speak_for_i32.
  ...
  Definition speak `{State.Trait} (self : ref Self) : M unit :=
    let* _ :=
      let* _ :=
        let* α0 := borrow [ mk_str "i32: "; mk_str "
" ] (list (ref str)) in
        let* α1 := deref α0 (list (ref str)) in
        let* α2 := borrow α1 (list (ref str)) in
        let* α3 := pointer_coercion "Unsize" α2 in
        let* α4 := deref self i32 in
...
Definition gre
  ...
  let* _ :=
    let* α0 := true in
    let* α1 := borrow (x, α0) (_ * bool) in
    clone.Speak.speak α1 in
  Pure tt.

Definition arg `{State.Trait} (x : i32) : M unit :=
  let* _ :=
    let* α0 := true in
    let* α1 := borrow (x, α0) (i32 * bool) in
    clone.Speak.speak α1 in
  Pure tt.
