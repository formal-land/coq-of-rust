...
Definition main `{ℋ : State.Trait} : M unit :=
  M.function_body
    (let* x : ltac:(refine u64) := M.alloc 4 in
    let* _ : ltac:(refine unit) :=
      let _ := InlineAssembly in
      M.alloc tt in
    let* _ : ltac:(refine unit) :=
      let* α0 : ltac:(refine (ref u64)) := borrow x in
      let* α1 : ltac:(refine u64) := M.alloc 4 in
      let* α2 : ltac:(refine u64) := M.alloc 6 in
      let* α3 : ltac:(refine u64) := BinOp.mul α1 α2 in
      let* α4 : ltac:(refine (ref u64)) := borrow α3 in
      let* α5 : ltac:(refine (M.Val ((ref u64) * (ref u64)))) :=
        M.alloc (α0, α4) in
      let* α6 := M.read α5 in
      match α6 with
      | (left_val, right_val) =>
