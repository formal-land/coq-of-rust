...
Definition main :=
  ...
  match (empty_array,[][struct RangeFull {} ]) with (left_val,right_val) => if unary (eq (unary left_val) (unary right_val)) then
    ...
    tt end ;;
  match into_iter (struct Range {start := 0;end := add (xs ) 1} ) with iter => loop match next iter with None [] => Break Some [0 : i] => match xs i with Some (xval) => $crate.io._print (new_v1 ["";": ";"\n"] [new_display i;new_display xval]) ;;
  tt None => $crate.io._print (new_v1 ["Slow down! ";" is too far!\n"] [new_display i]) ;;
  tt end end ;;
  tt from for end.