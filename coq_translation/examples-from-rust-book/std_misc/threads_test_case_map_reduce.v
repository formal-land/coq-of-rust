...
Definition main (_ : unit) :=
  ...
  let chunked_data := split_whitespace data in
  match into_iter (enumerate chunked_data) with
  | iter =>
    loop
            ...
            (fun  =>
              let result :=
                sum
                  (map
                    (chars data_segment)
                    (fun c => expect (to_digit c 10) "should be a digit")) in
              _crate.io._print
                (_crate::fmt::ImplArguments.new_v1
  ...
  let final_result :=
    sum (map (into_iter children) (fun c => unwrap (join c))) in
  _crate.io._print
    (_crate::fmt::ImplArguments.new_v1
      ["Final sum result: ";"\n"]
