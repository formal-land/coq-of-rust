...
Definition foo `{State.Trait} {A : Set} (o : core.option.Option A) : M unit :=
  match o with
...
Module tests.
  Definition test_file `{State.Trait} (_ : unit) : M unit :=
    let* file :=
...
".["as_bytes"] in
  ...
  Definition test_file_also `{State.Trait} (_ : unit) : M unit :=
    let* file :=
      let* α0 := std.fs.OpenOptions::["new"] tt in
      let* α1 := α0.["append"] true in
      let* α2 := α1.["create"] true in
