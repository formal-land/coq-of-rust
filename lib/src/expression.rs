...
fn compile_qpath(tcx: &TyCtxt, env: &mut Env, qpath: &QPath) -> Expr {
    match qpath {
        QPath::Resolved(_, path) => Expr::Var(compile_path(env, path)),
        QPath::TypeRelative(ty, segment) => {
            let ty = compile_type(tcx, env, ty);
...
pub(crate) fn compile_expr(tcx: &TyCtxt, env: &mut Env, expr: &rustc_hir::Expr) -> Expr {
    match &expr.kind {
        ExprKind::ConstBlock(_anon_const) => Expr::LocalVar("ConstBlock".to_string()),
        ExprKind::Array(elements) => {
            let elements = elements
                .iter()
                .map(|expr| compile_expr(tcx, env, expr))
                .collect();
            Expr::Array { elements }
        }
        ExprKind::Call(func, args) => {
            ...
            match func.kind {
                ...
                _ => {
                    let func = Box::new(compile_expr(tcx, env, func));
                    Expr::Call { func, args }
                }
            }
        }
        ExprKind::MethodCall(path_segment, object, args, _) => {
            let object = compile_expr(tcx, env, object);
        ...
        ExprKind::Binary(bin_op, expr_left, expr_right) => {
            let expr_left = compile_expr(tcx, env, expr_left);
            let expr_right = compile_expr(tcx, env, expr_right);
        ...
        ExprKind::Unary(un_op, expr) => {
            let expr = compile_expr(tcx, env, expr);
        ...
        ExprKind::Cast(expr, ty) => Expr::Cast {
            expr: Box::new(compile_expr(tcx, env, expr)),
        ...
        ExprKind::DropTemps(expr) => compile_expr(tcx, env, expr),
        ExprKind::Let(rustc_hir::Let { pat, init, .. }) => {
            let pat = compile_pattern(tcx, env, pat);
            let init = Box::new(compile_expr(tcx, env, init));
            Expr::LetIf { pat, init }
        }
        ExprKind::If(condition, success, failure) => {
            let condition = Box::new(compile_expr(tcx, env, condition));
            let success = Box::new(compile_expr(tcx, env, success));

            let failure = match failure {
                Some(expr) => Box::new(compile_expr(tcx, env, expr)),
                None => Box::new(tt()),
            };
            Expr::If {
                condition,
        ...
        ExprKind::Loop(block, label, loop_source, _) => {
            ...
            let body = Box::new(compile_block(tcx, env, block));
            let loop_source = compile_loop_source(loop_source);
            Expr::Loop { body, loop_source }
        }
        ExprKind::Match(scrutinee, arms, _) => {
            let scrutinee = Box::new(compile_expr(tcx, env, scrutinee));
            let arms = arms
                .iter()
                .map(|arm| {
                    let pat = compile_pattern(tcx, env, arm.pat);
                    let body = compile_expr(tcx, env, arm.body);
        ...
        ExprKind::Closure(rustc_hir::Closure { body, .. }) => {
            ...
            let args = body
                .params
                .iter()
                .map(|rustc_hir::Param { pat, .. }| compile_pattern(tcx, env, pat))
                .collect();
            let body = Box::new(compile_expr(tcx, env, body.value));
            Expr::Lambda { args, body }
        }
        ExprKind::Block(block, label) => {
            ...
            Expr::Block(Box::new(compile_block(tcx, env, block)))
        }
        ExprKind::Assign(left, right, _) => {
            let left = Box::new(compile_expr(tcx, env, left));
            let right = Box::new(compile_expr(tcx, env, right));
            Expr::Assign { left, right }
        }
        ExprKind::AssignOp(bin_op, left, right) => {
            let func = compile_assign_bin_op(bin_op);
            let left = compile_expr(tcx, env, left);
            let right = compile_expr(tcx, env, right);
            Expr::MethodCall {
                object: Box::new(left),
                func,
                args: vec![right],
        ...
        ExprKind::Field(base, ident) => {
            let base = Box::new(compile_expr(tcx, env, base));
        ...
        ExprKind::Index(base, index) => {
            let base = Box::new(compile_expr(tcx, env, base));
            let index = Box::new(compile_expr(tcx, env, index));
        ...
        ExprKind::Path(qpath) => {
            ...
            compile_qpath(tcx, env, qpath)
        }
        ExprKind::AddrOf(_, _, expr) => Expr::AddrOf(Box::new(compile_expr(tcx, env, expr))),
        ...
        ExprKind::Ret(expr) => {
            ...
            let args = match expr {
                Some(expr) => vec![compile_expr(tcx, env, expr)],
                None => vec![],
            };
            Expr::Call { func, args }
        }
        ...
        ExprKind::Struct(qpath, fields, base) => {
            ...
            let fields = fields
                ...
                .collect();
            let base = base.map(|expr| Box::new(compile_expr(tcx, env, expr)));
    ...
    match stmts {
        [stmt, stmts @ ..] => match stmt.kind {
            rustc_hir::StmtKind::Local(rustc_hir::Local { pat, init, .. }) => {
                let pattern = Box::new(compile_pattern(tcx, env, pat));
                let init = match init {
                    Some(init) => Box::new(compile_expr(tcx, env, init)),
                    None => Box::new(tt()),
                };
                let body = Box::new(compile_stmts(tcx, env, stmts, expr));
                Stmt::Let {
                    is_monadic: false,
                    pattern,
                    init,
            ...
            rustc_hir::StmtKind::Item(_) => compile_stmts(tcx, env, stmts, expr),
            rustc_hir::StmtKind::Expr(current_expr) | rustc_hir::StmtKind::Semi(current_expr) => {
                let first = Box::new(compile_expr(tcx, env, current_expr));
                let second = Box::new(compile_stmts(tcx, env, stmts, expr));
                Stmt::Let {
                    is_monadic: false,
        ...
        [] => Stmt::Expr(Box::new(match expr {
            Some(expr) => compile_expr(tcx, env, expr),
            None => tt(),
        })),
    }
}
...
fn compile_block(tcx: &TyCtxt, env: &mut Env, block: &rustc_hir::Block) -> Stmt {
    compile_stmts(tcx, env, block.stmts, block.expr)
}
