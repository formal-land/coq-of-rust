...
impl FunDefinition {
    ...
    fn to_coq(&self, name: &str) -> coq::TopLevel {
        coq::TopLevel::new(
            &[
...
impl TopLevelItem {
    fn to_doc(&self, to_doc_context: ToDocContext) -> Doc {
        match self {
            ...
            TopLevelItem::Definition { name, definition } => definition.to_doc(name),
            TopLevelItem::Module {
                name,
                body,
                is_dead_code,
            ...
            TopLevelItem::TypeStructTuple {
                ...
                coq::TopLevelItem::Definition(coq::Definition::new(
                    name,
                    &coq::DefinitionKind::Alias {
                        args: vec![if ty_params.is_empty() {
                            vec![]
                        } else {
                            vec![coq::ArgDecl::new(
                                &coq::ArgDeclVar::Simple {
                        ...
                        &coq::DefinitionKind::Alias {
                            ...
                            body: coq::Expression::just_name("M.Val").apply(
                                ...
                                .apply_many(
                                    &ty_params
                                        .iter()
                                        .map(|(ty_param, _)| coq::Expression::just_name(ty_param))
                                        .collect::<Vec<_>>(),
                                ),
                            ),
                        },
